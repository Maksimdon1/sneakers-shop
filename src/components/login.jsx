  ...
  return (
    ...
    <div className="login" id='box'  >
        ...
        if (width <= 723 && width >=250) {
          return (
            <div className="line" onTouchStart={(el)=>{start =  el.changedTouches[0].clientY}} onTouchEnd={(el)=>[end = el.changedTouches[0].clientY, chech()]}></div>
          )}
          else{
            return (
        ...
        <div className="input-container">
          <label>Пароль</label>
          <input type="password" id='password' autoSave='true' defaultValue={password} onChange={(el)=>{(setPassword(el.target.value))}}  name="password" autoComplete='current-password' required={true}/>
